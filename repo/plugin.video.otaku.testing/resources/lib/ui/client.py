...
def request(
    ...
    try:
        ...
        if not ssl_context_created:
            ...
            if not ssl_context_created and verify and node == 'XboxOne':
                try:
                    ssl_context = ssl.create_default_context()
                    ssl_context.maximum_version = ssl.PROTOCOL_TLSv1_3
    ...
    def _get_cookie(self, netloc, timeout):
        ...
        if resp:
            ...
            if soln.get('status') < 300:
                cookie = '; '.join([
                    '%s=%s' % (i.get('name'), i.get('value'))
                    for i in soln.get('cookies')
                    if i.get('name').startswith('cf')
                ])
